...
TreeModel::TreeModel(const QStringList &headers, const QString &data, QObject *parent)
    ...
    rootData << "Узел"
             << "Описание"; // В модели будет 2 столбца
    rootItem = new TreeItem(rootData);
    // Создали корневой элемент
    setupModelData_new(data.split(QString("\n")), rootItem);
...
void TreeModel::setupModelData_new(const QStringList &lines, TreeItem *parent)
{
    QList<TreeItem *> parents;
    QList<int> indentations;
    parents << parent;
        ...
        if (!line.isEmpty()) {
            ...
            for (int level = 0; level < position-1; level++) {
                ...
                for (int row = 0; row < parents.last()->childCount(); row++) {
                    TreeItem*  tree_item = parents.last()->child(row);
                    row_item = row;
                ...
                if (parents.last()->childCount() >= 0) {
                    parents << parents.last()->child(row_item);
                }
            }
